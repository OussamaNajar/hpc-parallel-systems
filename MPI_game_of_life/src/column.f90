!===============================================================================
! MPI Column Decomposition for Conway's Game of Life
!===============================================================================
!
! DESCRIPTION:
!   Implements a 1D vertical (column) decomposition. The global NxN grid is
!   split into vertical strips, with each rank owning a contiguous block
!   of columns. Communication occurs only with west/east neighbors.
!
! DECOMPOSITION STRATEGY:
!   - Rank k owns columns [my_col_start, my_col_start + my_local_cols - 1]
!   - my_local_cols = N/nprocs + (rank < N mod nprocs ? 1 : 0)
!   - my_col_start = 1 + rank*base + min(rank, remainder)
!
! GHOST CELL LAYOUT:
!   my_grid(0:N+1, 0:my_local_cols+1)
!   - Column 0: ghost column from west neighbor
!   - Column my_local_cols+1: ghost column from east neighbor
!   - Rows 0 and N+1: fixed boundary (always 0)
!
! COMMUNICATION PATTERN:
!   Each timestep:
!   1. Send my_grid(:,1) west, receive into my_grid(:,my_local_cols+1)
!   2. Send my_grid(:,my_local_cols) east, receive into my_grid(:,0)
!   3. Compute next generation using 8-neighbor stencil
!
! PERFORMANCE NOTES:
!   - Better cache utilization than row decomposition (column-major Fortran)
!   - Contiguous memory access for ghost column packing
!   - ~33.6x speedup at 32 ranks
!   - Second-best 1D decomposition after 2D methods
!
! BOUNDARY CONDITIONS:
!   - West/east boundaries: ghost exchange with neighbors
!   - North/south boundaries: fixed at 0 (rows 0 and N+1)
...
program Column_Decomposition
    ...
    base_cols = N / nprocs
    rem_cols = mod(N, nprocs)

    my_local_cols = base_cols + merge(1, 0, rank < rem_cols)
    my_j_start = 1 + rank * base_cols + min(rank, rem_cols)
    my_i_start = 1
    my_local_rows = N
    ...
    left = merge(rank - 1, MPI_PROC_NULL, rank > 0)
    right = merge(rank + 1, MPI_PROC_NULL, rank < nprocs - 1)
    ...
    do step = 1, STEPS
      ...
      call MPI_Sendrecv(my_grid(1,1), my_local_rows, MPI_INTEGER, left, 20, &
                        my_grid(1,my_local_cols+1), my_local_rows, MPI_INTEGER, right, 20, &
                        MPI_COMM_WORLD, status, ierr)

      call MPI_Sendrecv(my_grid(1,my_local_cols), my_local_rows, MPI_INTEGER, right, 21, &
                        my_grid(1,0), my_local_rows, MPI_INTEGER, left, 21, &
                        MPI_COMM_WORLD, status, ierr)
